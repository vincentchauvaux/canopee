...
export default function Agenda() {
  ...
  const [createClassInitialDate, setCreateClassInitialDate] = useState<
    string | null
  >(null);
  ...
  const openCreateClassForDay = (day: Date) => {
    setCreateClassInitialDate(format(day, "yyyy-MM-dd"));
    setClassModalOpen(true);
  };

  const handleCloseClassModal = () => {
    setClassModalOpen(false);
    setCreateClassInitialDate(null);
    void fetchClasses();
  };
    ...
    <section id="agenda" className="py-20 bg-accent">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          {isAdmin && (
            <p className="text-sm text-text-dark/60 max-w-2xl mx-auto mt-3">
              Administrateur : cliquez sur un jour du calendrier pour créer un
              cours à cette date (même formulaire que dans l&apos;admin).
            </p>
          )}
        </div>
      ...
      {isAdmin && classModalOpen && (
        <ClassFormModal
          classItem={null}
          isOpen={classModalOpen}
          onClose={handleCloseClassModal}
          initialDate={createClassInitialDate}
        />
      )}
    </section>
  );
